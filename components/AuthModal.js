...
const ModalContent = ({
  ...
  return React.createElement('div', {
    className: "bg-[#0F172A] p-10 rounded-[2.5rem] shadow-2xl w-full max-w-[420px] relative border border-white/10 animate-fade-in-up"
  },
    ...
    React.createElement('form', {
      ...
      !isLoginView && React.createElement('div', null,
        React.createElement('input', {
          ...
          className: "w-full px-6 py-4 bg-[#1E293B] border border-white/5 rounded-full focus:ring-2 focus:ring-brand-red/50 focus:outline-none text-white placeholder-slate-500 transition-all"
        })
      ),
      ...
      React.createElement('button', {
        type: "submit",
        disabled: loading,
        className: "w-full bg-gradient-to-r from-[#F34B4B] to-[#E11D48] hover:scale-[1.02] active:scale-[0.98] text-white font-bold py-5 px-6 rounded-full transition-all mt-4 flex items-center justify-center shadow-lg shadow-red-500/30 disabled:opacity-70"
    ...
    React.createElement('div', { className: "flex items-center my-10" },
      ...
      React.createElement('span', { className: "px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]" }, "OR"),
      React.createElement('div', { className: "flex-grow border-t border-white/10" }),
    ),

    React.createElement('button', {
      onClick: () => onLoginSuccess({ email: 'guest@jobinterview.com', fullName: 'Guest User' }),
      className: "w-full flex items-center justify-center py-5 px-6 bg-transparent border border-white/10 rounded-full hover:bg-white/5 transition-all text-white font-bold"
    }, 
    ...
    React.createElement('p', {
      ...
      React.createElement('button', {
        ...
        className: "font-bold text-[#F34B4B] hover:underline ml-1"
...
const AuthModal = ({ isOpen, onClose, onLoginSuccess, language, setView }) => {
  ...
  return React.createElement('div', {
    className: "fixed inset-0 bg-black/80 z-[100] flex justify-center items-center backdrop-blur-xl transition-all p-4"
  },
